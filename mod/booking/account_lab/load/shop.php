...
<table width="100%" border="0" cellspacing="2" cellpadding="2">
  <tbody>
    <tr>
      <td class="font-28">รายจ่าย เดือน <?=$_GET[month] ?> ปี    <?=$_GET[year] ?></td>
    </tr>
  </tbody>
</table>
...
$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
$query = "SELECT SUM(price_all_total) FROM order_booking  where transfer_date like '%" . $no . "%'  and status = 'CONFIRM' "; 
...
$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
$query = "SELECT SUM(price_park_total) FROM order_booking  where transfer_date like '%" . $no . "%'  and status = 'CONFIRM' "; 
$result = mysql_query($query) or die(mysql_error());
// Print out result
while($row = mysql_fetch_array($result)){
...
$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
$query = "SELECT SUM(price_person_total) FROM order_booking  where transfer_date like '%" . $no . "%'  and status = 'CONFIRM' "; 
$result = mysql_query($query) or die(mysql_error());
// Print out result
while($row = mysql_fetch_array($result)){
...
$data_sale="where  status = 'CONFIRM'  and transfer_date='".$arr[timeline][dayall]."'";
...
$('#show_price_<?=$arr[timeline][dayall]?>').click(function(){  

 $( "#load_mod_popup_3" ).toggle();
	
 var url_load = "load_page_mod_3.php?name=booking/load&file=work_all&type=edit&action=confirm&day=<?=$arr[timeline][dayall]?>";
...
<table width="100%"  border="0" cellspacing="0" cellpadding="2">
  <tr align="center">
    <td width="100" style="height:35px; "  class="font-16">
	...
	<a id="show_price_<?=$arr[timeline][dayall]?>" class="font-16">
    
    <span  class="font-16">
	
	<?=$arr[timeline][dayall]?> </span>
    ...
    <td width="60" align="right"  class="font-16" style="display:nones"><?= number_format($park, 0 );?></td>
    <td width="60" align="right"  class="font-16"><?= number_format($person, 0 );?></td>
