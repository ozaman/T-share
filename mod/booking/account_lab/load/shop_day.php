...
<table width="100%" border="0" cellspacing="2" cellpadding="2">
  <tbody>
    <tr>
      <td class="font-28">รายจ่ายวันที่ <?=$_GET[day] ?></td>
    </tr>
  </tbody>
</table>
 ...
 <table width="100%"  border="0" cellspacing="1">
   <tr align="center">
    <td width="100" align="center" bgcolor="#999999"  class="font-16" style="height:30px; font-size:16px "><font color="#FFFFFF">เลขจอง</strong></td>
    <td width="60" align="center" bgcolor="#999999"  class="font-16" style="height:30px; font-size:16px ; display:noneห "><font color="#FFFFFF">ค่าจอด</strong></td>
    <td width="60" align="center" bgcolor="#999999"  class="font-16" style="height:30px; font-size:16px "><font color="#FFFFFF">ค่าหัว</strong></td>
...
$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
$query = "SELECT SUM(price_all_total) FROM order_booking  where   transfer_date='".$no."'  and status = 'CONFIRM' "; 
...
$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
$query = "SELECT SUM(price_park_total) FROM order_booking  where transfer_date='".$no."'  and status = 'CONFIRM' "; 
$result = mysql_query($query) or die(mysql_error());
// Print out result
while($row = mysql_fetch_array($result)){
...
$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
$query = "SELECT SUM(price_person_total) FROM order_booking  where transfer_date='".$no."'  and status = 'CONFIRM' "; 
$result = mysql_query($query) or die(mysql_error());
// Print out result
while($row = mysql_fetch_array($result)){
...
$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
$res[timeline] = $db->select_query("SELECT * FROM  order_booking  where  transfer_date='".$no."'   order by  id  asc limit 200 ");
...
//$arr[timeline][dayall]='2017-07-13';
 ...
 $data_sale="where  status = 'CONFIRM'  and transfer_date='".$no."'";
...
<table width="100%"  border="0" cellspacing="0" cellpadding="2">
  <tr align="center">
    <td width="100" style="height:35px; "  class="font-16"><?=$arr[timeline][invoice]?></td>
    <td width="60" align="right"  class="font-16" style="display:nones"><?= number_format($arr[timeline][price_park_total], 0 );?></td>
    <td width="60" align="right"  class="font-16"><?= number_format($arr[timeline][price_person_total], 0 );?></td>
    <td align="right"  class="font-16" style="padding-right:10px; "> <B>
 
	<?= number_format( $arr[timeline][price_all_total] , 0 );?>
...
<table width="100%"  border="0" cellspacing="2" cellpadding="2"  >
  <tr  >
    <td  class="font-16" style="height:30px; padding-left:5px; ">ค่าจอด</td>
    <td width="40%" align="right"  class="font_24" style="padding-right:10px; color:#000000 "> 
...
<table width="100%"  border="0" cellpadding="2" cellspacing="2"  style="background-color:#F6F6F6;border:  solid 1px #999999;  ">
  <tr  >
     <td  class="font-16" style="height:30px; padding-left:5px; ">รายจ่ายรวม</td>
