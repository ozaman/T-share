  ...
  <script>
 var url_driver_topoint_<? echo $arr[project][id];?>= "popup.php?name=booking/load/form&file=checkin_status&id=<? echo $arr[project][id];?>&type=check_driver_topoint&time=<?=$arr[project][date_driver_topoint]?>&status=<?=$arr[project][check_driver_topoint]?>";
  ...
  <table width="100%" border="0" cellspacing="2" cellpadding="0" class="div-all-checkin border-alert" id="box_driver_topoint_<?=$arr[project][id];?>">
        <tbody>
          ...
          <tr>
            ...
            <i  id="photo_driver_topoint_<?=$arr[project][id];?>" class="fa  fa-camera" style="color:<?=$main_color?>; font-size:16px; border-radius: 50%; padding:5px; border: solid 2px <?=$main_color?>  "></i>
