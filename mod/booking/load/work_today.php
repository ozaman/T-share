<?
  ...
  ?>
    ...
    <script>
  $(".text-topic-action-mod-3").html('งานตอนนี้');
 ...
 .div-all-work{
	 padding:5px;   border-radius: 0px; border: 1px solid #ddd;   background:<? echo $bgcolor; ?>; margin-bottom:20px; box-shadow: 0px  0px 10px #DADADA  ;
...
$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
 ...
 /// $numday_work = $db->num_rows('order_booking',"id","transfer_date='".$_GET[day]."' $filter limit 1 order");
...
$('#booking_payment_<?=$arr[project][id]?>').hide(); 
  ...
  /////
   ...
   <div  class="div-all-work" style=" border: 0px solid <?= $bgcolor?>;" >
...
<li><a id="booking_payment_<?=$arr[project][id]?>" class="font-22" style="color:#FFFFFF; font-size:22px"><i class="fa fa-thumbs-up"></i>ชำระเงิน</a></li> 
 ...
 //alert(0);
    ...
    <div id="main_data_booking_div_<?=$arr[project][id]?>" class="box-border"> <?  include ("mod/booking/load/form/data.php");?></div>
  ...
  <div id="main_driver_booking_div_<?=$arr[project][id]?>" class="box-border" style="display: none;"> <?   include ("mod/booking/load/form/driver.php");?></div>
	 ...
	 <?  include ("mod/booking/load/form/checkin_driver.php");?>
