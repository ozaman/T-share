...
<table width="100%" border="0" cellpadding="1" cellspacing="5" id="table_show_hide_checkin_<?=$arr[project][invoice];?>">
  ...
  <tr id="booking_topoint_<?=$arr[project][id]?>" style="display:nones">
    <td width="100" class="font-22">
    
    <? 
    if($arr[project][check_driver_topoint]==1 ){ ?>
 
      <script> 
   $("#check_driver_topoint_<?=$arr[project][id]?>").removeClass('disabledbutton');
    ...
    <button id="check_driver_topoint_<?=$arr[project][id]?>" type="button" class="btn  btn-info "  style="width:100%;text-align:left;padding:5px "><span class="font-22" >
    <i class="fa fa-map-marker" style="width:20px;"></i>แจ้งถึงแล้ว</span>
    </button>
    ...
    <td class="font-22">
     ...
     <? if($arr[project][check_driver_topoint]==1){ ?>
        <script>
		$("#check_driver_topoint_<?=$arr[project][id]?>").removeClass('btn-info');
	 $("#check_driver_topoint_<?=$arr[project][id]?>").addClass('disabledbutton-gray');

  </script>
        <i class="fa   fa-check-circle " style="width:20px; color:#39B54A"></i> <b>
        <strong><?=$date = date('H:i:s',  $arr[project][driver_complete_date]);?></strong>
        <? }  else {?>
        <i class="fa  fa-circle-o-notch fa-spin 6x" style="color:#FF0000"></i> <strong>รอดำเนินการ</strong>
        <? } ?>
      </span></td>
  </tr>
 
  <tr id="booking_get_guest2_<?=$arr[project][id]?>" style="display:nones">
    <td width="100" class="font-22">
    <button id="complete_get_guest_<?=$arr[project][id]?>" type="button" class="btn disabledbutton-gray"  style="width:100%;text-align:left;padding:5px;">
    <span class="font-22"><i class="fa  fa-users" style="width:20px;"  ></i> รับแขกแล้ว</span>
    </button>
    </td>
    <td class="font-22"><? 
    if($arr[project][driver_topoint]==1 and  $arr[project][reception_get_guest]==0){ ?>
    
      <? } ?>
      <span  id="time_complete_get_guest_<?=$arr[project][id]?>">
        <? if($arr[project][reception_get_guest]==1){ ?>
  ...
  <tr style="display:nones">
    ...
    <td width="100" class="font-22"><button  class="btn disabledbutton-gray " style="width:100%;text-align:left;padding:5px ">
    <span class="font-22"><i class="fa  fa-user" style="width:20px;"  ></i>ลงทะเบียน</span></button></td>
    <td class="font-22"><? if($arr[project][driver_topoint]==1 and  $arr[project][driver_register]==0){ ?>
      <script> 
   $("#complete_booking_data_<?=$arr[project][id]?>").removeClass('disabledbutton');
      ...
      <? } ?>
      <span  id="time_complete_booking_data_<?=$arr[project][id]?>">
        <? if($arr[project][driver_register]==1){ ?>
        <script>
  ...
  <tr style="display:nones">
    ...
    <td width="100" class="font-22"><button  class="btn disabledbutton-gray " style="width:100%;text-align:left;padding:5px "><span class="font-22"><i class="fa fa-file-text" style="width:20px;"  ></i> แจ้งยอด</span></button></td>
    <td class="font-22"><? if($arr[project][driver_topoint]==1 and  $arr[project][driver_register]==0){ ?>
      <script> 
   $("#complete_booking_data_<?=$arr[project][id]?>").removeClass('disabledbutton');
...
<? if($arr[project][plan_id]==7){
	$display_none = "";
}else{
	$display_none = "display:none;";
}?>
  <tr style="<?=$display_none;?>">
  
    <td width="100" class="font-22"><button  class="btn disabledbutton-gray " style="width:100%;text-align:left;padding:5px "><span class="font-22"><i class="fa fa-exchange" style="width:20px;"  ></i> โอนเงิน</span></button></td>
    <td class="font-22"><? if($arr[project][driver_topoint]==1 and  $arr[project][driver_register]==0){ ?>
      <script> 
   $("#complete_booking_data_<?=$arr[project][id]?>").removeClass('disabledbutton');
   ...
   $("#tr_booking_payment_<?=$arr[project][id]?>").show();
	...
	$("#text_payment_booking_data_<?=$arr[project][id]?>").html('<i class="fa  fa-folder-open"  style="width:20px;" ></i> รับเงินแล้ว</span>');
	 ...
	 $("#payment_booking_data_<?=$arr[project][id]?>").addClass('disabledbutton-gray');
      ...
      <i class="fa   fa-check-circle " style="width:20px; color:#39B54A"></i> <b>
     <strong> <?=$date = date('H:i:s',  $arr[project][driver_payment_date]);?></strong>
      <?  } ?>
  ...
  <? } ?>
		...
		<script>
			 ...
			 $("#complete_get_guest_<?=$arr[project][id]?>").click(function(){ 
  
 
  
	   swal({
		title: "<font style='font-size:28px'><b> คุณแน่ใจหรือไม่",
		text: "<font style='font-size:22px'>ว่าแผนกต้อนรับ รับแขกแล้ว",
	type: "info",
  ...
  var url_<?=$arr[project][id]?> = "popup.php?name=booking&file=savedata&action=get_guest&type=driver&id=<?=$arr[project][id]?>";
  
	$.post( url_<?=$arr[project][id]?>, function( data ) {
	 	console.log(data);
	});
 $("#complete_get_guest_<?=$arr[project][id]?>").removeClass('btn-info');
  $("#complete_get_guest_<?=$arr[project][id]?>").addClass('disabledbutton-gray');
  $("#complete_get_guest_<?=$arr[project][id]?>").css('background-color','');
  
 
 $("#time_complete_get_guest_<?=$arr[project][id]?>").html('<i class="fa   fa-check-circle " style="width:20px; color:#39B54A"></i><b> <?=$date = date('H:i:s');?>');
