...
$arr[project][id]=$_GET[id];
 ...
 $('#btn_checkin_popup_<?=$_GET[id]?>').click(function(){   
  ...
  var url_<?=$arr[project][id]?> = "popup.php?name=booking/load/form&file=savedata&action=<?=$action;?>&type=<?=$_GET[type]?>&id=<?=$arr[project][id]?>&lat="+lat+"&lng="+lng;
  
  console.log(url_<?=$arr[project][id]?>);
//  return;

 $('#status_<?=$_GET[type];?>_<?=$arr[project][id]?>').load(url_<?=$arr[project][id]?>);
