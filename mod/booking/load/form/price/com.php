  ...
  $arr[pay_com] = $db->fetch($res[pay_com]);
  		if($arr[pay_com][driver_approve]>0){
			...
			$txt_pay = '<font color="#59AA47;"><b>ได้รับเงินแล้ว</b></font>';
  			...
  			$alert_history = "swal('ประวัติ' ,'จ่ายเงินเมื่อวันที่ ".date('Y-m-d H:i:s', $arr[pay_com][last_update])." น.' ,'success');";
		}else{
...
<table width="100%" border="0" cellspacing="1" cellpadding="1"  class="div-all-price">
  <tbody>
        ...
        <tbody>
          <tr>
            ...
            <td class="font-24"><table width="100%" border="0" cellspacing="0" cellpadding="0">
              <tbody>
                <tr>
                  <td class="font-24"><font color="#333333" ><b>ค่าคอมมิชชั่น</b></font></td>
                </tr>
                <tr>
                  <td class="font-22">โอนเงินเข้าบัญชี <br>(แขกเดินทางกลับ)</td>
                </tr>
              </tbody>
            </table></td>
    ...
    <tr>
      <td valign="middle" class="font-26" id="txt_pay_com_<?=$arr[project][id];?>"><?=$txt_pay;?></td>
    ...
    <tr style="display:noneS">
      ...
      <span class="font-26"><b><i class="fa  fa-circle-o-notch fa-spin 6x" style="color:#FF0000"></i></b>
...
<script>
	$.ajax({
		    ...
		    error: function()
		    {
		        ...
		        $('#btn_com_doc_<?=$arr[project][id]?>').attr('onclick','swal("ไม่มีการอัพโหลดไฟล์!", "", "error")');
		    },
		    success: function()
		    {
...
<? }
  ...
  if($check_park>0){
  		...
  		$alert_history = "swal('ประวัติ' ,'จ่ายเงินเมื่อวันที่ ".date('Y-m-d H:i:s', $arr[com_park][last_update])." น.' ,'success');";
  }	else{
  		$btn_row_approve = '';
...
(แขกเดินทางกลับ)</td>
    ...
    <tr style="display:noneS">
      <td valign="middle" class="font-26"><b><font >จำนวนเงิน</font></b></td>
      <td align="right" class="font-20" id="price_com_cost_payment_<?=$arr[project][id];?>"><span class="font-26"><b>
    ...
    <tr>
      <td colspan="2" valign="middle"><table width="100%" border="0" cellspacing="2" cellpadding="2">
        <tbody>
           <tr id="show_com_his_<?=$arr[project][id]?>">
            <td width="50%">
            <button  id="btn_com_doc_<?=$arr[project][id]?>" onclick=""  type="button" class="btn btn-default"  style="width:100%;text-align:left;padding:5px;  ;  border-radius: 3px; border:none; background-color:#FFF "><center><span class="font-22"><i class="fa fa-folder-open" style="width:24px;color:<?=$main_color?>"  ></i>เอกสาร 
