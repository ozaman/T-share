...
$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
 ...
 <div style="border: 1px solid #ddd;
    ...
    margin-bottom: 10px;
    color: #3b5998;
    box-shadow: 0px 0px 10px #cccccc;display: none;" id="show_work_this_day">
 	<strong><?=$numday_work." งาน";?></strong>
 </div>
 <? if($numday_work==0){ ?>
 
 
 <div class="font-22"  style='background-color:#FFFFFF; padding:15px; border-radius:  15px;border: solid 1px #DADADA; margin-top:10px; '><center>
...
$arr[plan] = $db->fetch($res[plan]);
 ...
 <table width="100%" border="0" cellspacing="1" cellpadding="1" style="padding:5px; margin-top:3px;  border-radius: 15px; border: 1px solid #ddd;   background:#FFFDE9;    ">
  <tbody>
    <tr>
      <td height="40" id="status_<?=$arr[project][id]?>"><font color=""></font>
...
<div id="main_sub_booking_div_<?=$arr[project][id]?>" style="display:nones">
 ...
 <div id="main_back_booking_div_<?=$arr[project][id]?>" class="box-border"  style=" display:none;"> <?  include ("mod/booking/load/form/back.php");?></div>
  ...
  <div id="main_driver_booking_div_<?=$arr[project][id]?>" class="box-border" style="display: nones;"> <?  include ("mod/booking/load/form/driver.php");?></div>
     ...
     <div id="main_price_booking_div_<?=$arr[project][id]?>" style="display:nones" class="box-border"> <?    include ("mod/booking/load/form/price.php");?></div>
...
///  $('#status_<?=$arr[project][id]?>').html('<font style="font-size:22; color:#FF0000"><b>ยกเลิก</font>'); 
 ...
 $("#complete_booking_data_<?=$arr[project][id]?>").removeClass('disabledbutton');
  ...
  var url_<?=$arr[project][id]?> = "popup.php?name=booking&file=savedata&action=complete2&type=driver&id=<?=$arr[project][id]?>";
