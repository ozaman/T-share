...
<input  name="edit_all_car"  type="hidden" class="form-control"  id="edit_all_car" value="<?=$all_car?>"   />
<div  style="padding:5px;   border-radius: 5px; /*border: 1px solid #ddd;background-color:#F6F6F6;*/  margin-bottom: 0px; box-shadow: 0px  0px 0px #DADADA  ; margin-top: 15px;"  >
 ...
 if($arr[car][car_color]=='Silver Bronze'){ echo 'selected=selected';} ?>>บรอนด์เงิน</option>
	...
	<div class="font-26" style="margin-top:10px;" ><table width="100%" border="0" cellspacing="0" cellpadding="0">
		...
		while($arr[projectcar] = $db->fetch($res[projectcar])){
			...
			?>
			<script>
				$("#edit_use_car_<?=$arr[projectcar][id]?>").click(function(){
						///
						swal({
			...
			<script>
				$("#edit_cancel_car_<?=$arr[projectcar][id]?>").click(function(){
						///
						swal({
								title: "<font style='font-size:28px'><b> คุณแน่ใจหรือไม่",
 ...
 background:<? echo $bgcolor; ?>; margin-bottom:5px; box-shadow: 0px  0px 5px #DADADA; margin-top:10px; margin-bottom:20px;"    >
				...
				<a id="edit_car_<?=$arr[projectcar][id]?>"    style="text-decoration:none; margin-top:30px;" >
 ...
 if($arr[projectcar][plate_color]=="Green"){
	 echo "FFFFFF"; } ?>"  class="font-28"><b><? echo $arr[projectcar][plate_num];?> <br>
					...
					<td valign="top" style="display: none;"><img src="../data/pic/car/<?=$arr[projectcar][id];?>_1.jpg?v=<?=$arr[projectcar][update_date];?>"  width="100%"   border="0"      style="margin-top: 3px;border-radius:5px;" /></td>
							<td width="40" align="center">
							<input  name="edit_check_use_car" type="radio" id="edit_check_use_car_<?=$arr[projectcar][id]?>" style="height:30px; width:30px;box-shadow:none" value="CONFIRM"	/>
 ...
 $('#edit_check_use_car_<?=$arr[projectcar][id]?>').iCheck({
      ...
      increaseArea: '20%' // optional
									...
									$('#edit_car_<?=$arr[projectcar][id]?>').on('click', function () {
											
										
										
											  $('#edit_check_use_car_<?=$arr[projectcar][id]?>').iCheck('check'); 
											  
										//	document.getElementById('check_use_car_<?=$arr[projectcar][id]?>').checked=true;
										});
								
								   $('#edit_check_use_car_<?=$arr[projectcar][id]?>').on('ifChecked', function(event){	
							
											
											console.log('<?=$arr[projectcar][id]?>');
											$('#edit_check_use_car_id').val('<?=$arr[projectcar][id]?>');
			...
			<input type="hidden" id="txt_plate_edit_<?=$arr[projectcar][id]?>" value="<? echo $arr[projectcar][plate_num]." ".$arr[projectcar][province];?>"/>
			<? } ?>
		<input name="edit_check_use_car_id" type="hidden"  required="true" class="form-control" id="edit_check_use_car_id" style="padding:4px 2px;width:100%;"   value="<?=$arr[project][wait_status]?>" >
		<input name="edit_check_all_car" type="hidden"  required="true" class="form-control" id="edit_check_all_car" style="padding:4px 2px;width:100%;"   value="<?=$all_car?>"   >
...
if($all_car ==1){  ?>
	<script>
		$('#edit_car_<?=$arr[car][id]?>').click() ;
	</script>
	<? } ?>   
	
<script>
	var car_select = '<?=$_GET[car];?>';
	if(car_select!="" && car_select!=undefined){
		$('#edit_car_'+car_select).click() ;
	}
</script>