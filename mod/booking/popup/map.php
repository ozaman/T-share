...
$('#dv_map').addClass("load_preview_map");
 ...
 $("#text_mod_topic_action_map-txt" ).html("<?=$arr[shop][topic_th]?>");

  </script> 

 <div id="dv_map" style="margin-top: 50px;">
 <iframe width="100%"  height="95%"  frameborder="0" style="border:0" src="https://www.google.com/maps/embed/v1/directions?key=<?=$google_api?>&destination=<?=$arr[shop][address]?>&origin=<?=$_GET[lat]?>,<?=$_GET[lng]?>&center=<?=$_GET[lat]?>,<?=$_GET[lng]?>&avoid=tolls|highways&zoom=12" allowfullscreen></iframe>
