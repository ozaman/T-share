...
// echo $_GET[driver];
   ...
   $all_car = $db->num_rows('web_carall',"id","drivername='".$_GET[driver]."' ");
   $db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
   $res[car] = $db->select_query("SELECT * FROM   web_carall  where drivername ='".$_GET[driver]."' and status=1 order by id desc  ");
...
<div  style=" border-radius: 5px; /*border: 1px solid #ddd;background-color:#F6F6F6;*/  margin-bottom: 0px; box-shadow: 0px  0px 0px #DADADA  ;"  >
   ...
   <div class="font-26"  >
      ...
      <?
         ...
         $db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
         //// last id
         $res[projectcar] = $db->select_query("SELECT * FROM   web_carall  where drivername= '".$_GET[driver]."' and status=1   order by status_usecar desc  ");
         ///$res[contact_phone] = $db->select_query("SELECT * FROM  contact_phone WHERE company = '".$arr[product][admin_company]."' ");
               ...
               <tbody>
                  ...
                  <tr>
                        ...
                        <table width="100%" cellpadding="1" cellspacing="2"  >
                           <tr>
                              ...
                              <td width="100"   align="center" bgcolor="#<?=$plate_color?>" style="border: solid 2px; height:20px; color:#DADADA; padding:5px; padding-right:0px;border-radius:5px;"><font color="#<?
                                 ...
                                 <font   class="font-20"><? echo $arr[projectcar][province];?></font></b></font>
                              </td>
                           </tr>
                        </table>
                     ...
                     <td valign="top" style="display: none;">
                        <img src="../data/pic/car/<?=$arr[projectcar][id];?>_1.jpg?v=<?=$arr[projectcar][update_date];?>"  width="100%"   border="0"      style="margin-top: 3px;border-radius:5px;" />
                     </td>
      ...
      <i class="fa fa-plus-circle"  style="color:<?=$main_color?>" ></i>  <?=t_add_new_car;?>
      </a>
      <span style="color:#FF0000" class="font-22">
      <?
         if($all_car>0){ ?>
      (<?=t_number_of_using_car;?>  <?=$all_car?> <?=t_car;?>)
      <? }
         else{ ?>
      (<?=t_not_have_car;?>)
      <? } ?>
