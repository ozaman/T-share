...
<div   class="box box-default" style="margin-top:10px; margin-bottom:-10px; height:40px; padding-bottom:10px; padding-left:5PX; background-color:#F5F5F5" >


   <table width="100%"  border="0" cellspacing="0" cellpadding="0" style="margin-top:5px;">
       <tr>
         <td width="135"  class="font_16"  ><?
include "mod/load/contact/phone.php" ;
?></td>
         <td  class="font_16"><?
include "mod/load/contact/network.php" ;
?></td>
       </tr>
     </table>
</div>
  ...
  <tr>
    <td ><div style="background-color:#F6F6F6; margin-top:5px; margin-bottom:5px; padding: 2px 0px 2px 0px;"   >
      <table width="100%"  border="0" cellspacing="0" cellpadding="0">
        <tr>
          <td width="140" valign="top"><div class="topictransfer" style=" margin-top:-5px;" ><i class="fa  fa-map-marker" style="color:#c1c1c1"></i> <span class="font_18">สถานที่รับ</span></div></td>
          <td width="40"> <? if($arr_t_topic[$arr[project][pickup_place]] != ''){ ?>
        <? if( $arr_map[$arr[project][pickup_place]] != '' ){   ?>
        <a class="test" data-toggle="tooltip" title="แสดงแผนที่นำทาง"><img src="images/icon/view/map.png"   align="bottom"  width="32"  id="map_pickup"   />
        <? } ?>
      ...
      <? if( $arr_map[$arr[project][pickup_place]] != '' ){   ?>
		...
		<? if($arr_map[$arr[project][pickup_place]]==""){ ?>
		<a data-toggle="tooltip" title="ไม่มีแผนที่นำทาง"><img src="images/icon/view/no/map.png"   align="bottom"  width="32"    /></a>
...
<a  href="tel:<?=$arr[phone][phone]?>"  target="_blank" class="test" data-toggle="tooltip" title="โทรออก"><img src="images/icon/view/phone.png" width="32"   align="bottom"  /></a>
 <? } ?>
		<? if($arr[phone][phone]==""){  ?>
 <a data-toggle="tooltip" title="ไม่มีเบอร์ติดต่อ"><img src="images/icon/view/no/phone.png" width="32"   align="bottom"  /> </a>
 <? } ?>
  ...
  <tr>
    <td ><div style="background-color:#F6F6F6; margin-top:5px; margin-bottom:5px; padding: 2px 0px 2px 0px;"   ><table width="100%"  border="0" cellspacing="0" cellpadding="0">
      <tr>
        ...
        <td width="40"><? if($arr_t_topic[$arr[project][to_place]] != ''){ ?>
            <? if( $arr_map[$arr[project][to_place]] != '' ){   ?>
            <a class="test" data-toggle="tooltip" title="แสดงแผนที่นำทาง"   ><img src="images/icon/view/map.png"   align="bottom"  width="32"  id="map_toplace"   />
            <? } ?>
            <? if( $arr_map[$arr[project][to_place]] != '' ){   ?>
            </a>
            <? } ?>
...
?>
		...
		<? if($arr[phoneto][phone]<>""){  ?>
<a href="tel:<?=$arr[phoneto][phone]?>"  target="_blank" class="test"  data-toggle="tooltip" title="โทรออก"><img src="images/icon/view/phone.png" width="32"   align="bottom" id="no_phone_toplace" /></a>
 <? } ?>
		<? if($arr[phoneto][phone]==""){  ?>
<a data-toggle="tooltip" title="ไม่มีเบอร์ติดต่อ"><img src="images/icon/view/no/phone.png" width="32"   align="bottom"  /> </a>
 <? } ?>
 ...
 $('#tab_pickup_<?=$arr[project][id];?>').removeClass("tab_alert");
        ...
        <table  cellpadding="0" cellspacing="0">
				...
				<tr id="tr_btn_pickup2<?=$arr[project][id];?>"  style="display: none<? if($arr[project][driver_pickup] == 2){ ?> 1  <? } ?>">
					<td><button style="background-color: #ff2b2b;width: 125px;height:30px; border:none;color: #fffbfb;" class="btnstatus" data_id="<?=$arr[project][id];?>" col_name="driver_pickup" id="btn_pickup<?=$arr[project][id];?>2" data_val="2" <? if($arr[project][driver_pickup] == 2){ ?> disabled="disabled"  <? } ?> > ไม่เจอแขก</button></td>
				</tr>
			</table>
<?		
	}else{
...
<table  cellpadding="0" cellspacing="0">
				...
				<tr id="tr_btn_pickup2<?=$arr[project][id];?>"  style="display: nonea<? if($arr[project][driver_pickup] == 2){ ?> 1  <? } ?>">
					<td><button style="background-color: #ff2b2b;width: 125px;height:30px; border:none;color: #fffbfb;" class="btnstatus" data_id="<?=$arr[project][id];?>" col_name="driver_pickup" id="btn_pickup<?=$arr[project][id];?>2" data_val="2"  > ไม่เจอแขก</button></td>
				</tr>
			</table>
<?		
	}
...
?>	
  ...
  <? } ?>	
	...
	<tr>
		<td><div class="topictransfer">ระบุสาเหตุอื่น ๆ </div><input class="form-control"  type="text"  name="driver_remark_noguest_other" id="driver_remark_noguest_other"/></td>
	</tr>
	<tr>
		<td><div class="topictransfer">รูปภาพประกอบ (ถ้ามี)</div> 
	    <input type="file" class="form-control" name="driver_remark_noguest_file" id="driver_remark_noguest_file" /></td>
	</tr>
</table>
...
/////////////////// ปิดคำสั่ง 1 
	...
	//
			...
			create:function () {
        	$(this).closest(".ui-dialog").find('button:contains("ข้อมูลถูกต้อหง")').addClass("custom");
        	$(this).closest(".ui-dialog").find('button:contains("ข้อมูลไม่ถูกต้อ")').addClass("closed");
    		}
	});
...
<style>
 ...
 <!------ end---->  
  ...
  <? include "mod/view/loadmap.php"; ?></div>