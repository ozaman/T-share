...
$select_order="id,invoice,program,orderid,pickup_place,to_place,carno,cartype,drivername,air,airin_time,airout_h,airout_m,airout_time,driver_remark,total,guest_name,guest_phone,server,agent";
 $select_order="id";
$admincompany="1";
$agent="13";
...
<select name="drivername"  onchange="img_driver('<?=$name;?>',this.value)" id="drivername<?=$_GET[data_for]?>"  class="chosen-selects"  style="font-size:22px; padding-bottom: 1px; background-color:#FFFDE9; height:40px; width:100%;" chosen width="'100%'">
                      ...
                      <?
                      $db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
                      $res[category] = $db->select_query("SELECT * FROM ".TB_driver." where id > 0 and status=1    and nickname<>'' and company<>'' ORDER BY abs(company) asc, convert(nickname using tis620)  desc limit 1");
