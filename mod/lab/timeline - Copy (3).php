...
$select_order="id,invoice,program,orderid,pickup_place,to_place,carno,cartype,drivername,air,airin_time,airout_h,airout_m,airout_time,driver_remark,total,guest_name,guest_phone,server,agent,total,adult,child,baby,name_pickup_place,name_to_place,product_name,product_area";
// $select_order="id";
$admincompany="1";
$agent="13";
 $db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
 
 		///$db->del('transfer_report_all_day',"transfer_date<>'". $daywork."'"); 
...
$res[project] = $db->select_query("SELECT $select_order FROM  transfer_report_all_day  where id > 0  $findpart  and admin_company =1 and product_area='In'  and transfer_date='". $daywork."'  and status = 'CONFIRM'  and airout_time <>''  and carno >0 and drivername > 0  $findwork  group by invoice order by  airout_time ASC  LIMIT 100  ");
...
<select name="drivername"  onchange="img_driver('<?=$name;?>',this.value)" id="drivername<?=$_GET[data_for]?>"  class="chosen-selects"  style="font-size:18px; padding-bottom: 1px; background-color:#FFFDE9; height:40px; width:100%;" chosen width="'100%'">
                      <option value="" selected="selected">-- เปลี่ยนคนขับ --</option>
                      <?
                      $db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
                      $res[category] = $db->select_query("SELECT * FROM ".TB_driver." where  status=1    and nickname<>'' and company<>'' ORDER BY abs(company) asc, convert(nickname using tis620)  desc  ");
while ($arr[category] = $db->fetch($res[category])){
	...
	<div  style="margin-top:0px; font-size:22px; font-weight:bold;  padding:0PX; margin-right:16px; height: 30px;">
  <select name="drivername[]"  onchange="img_driver('<?=$name;?>',this.value)" id="drivername<?=$_GET[data_for]?>"  class="form-control"  style="font-size:22px; padding-bottom: 0px; background-color:#FFFDE9;height: 45px;">
                      <option value=""  >-- เปลี่ยนงาน --</option>
                      <?
                      ///$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
$db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
		mysql_query("SET NAMES UFT8"); 
...
$res[work] = $db->select_query("SELECT * FROM ".TB_transfer_report_all."   where  admin_company =1   and transfer_date='". $daywork."'  and status = 'CONFIRM'  and airout_time <>'' and  carno >0 and drivername > 0  $findwork  order by  carno,airout_time ASC    ");
  ...
  -ms-transform:rotateX(180deg); z-index:1 "></i>
			...
			<? }   else { ?> </font><font style="font-size:13px "><? echo "&nbsp;ยังไม่มี"; } ?></font>
                ...
                <div class="timeline-header"  style="padding-top:10px; padding-bottom:5px; "  >
				 ...
				 <?  include ("mod/lab/check_step.php");?>
...
<table width="100%"  border="0" cellspacing="0" cellpadding="0" style="margin-bottom:5px; ">
  ...
  <tr>
    <td><?=$arr[project][name_to_place] ?></td>
          ...
          <li style="display:none " class="show_air_out">
			...
			/*
   $db->connectdb(DB_NAME_DATA,DB_USERNAME,DB_PASSWORD);
    $db->update_db('transfer_report_all_day',array( 
		...
		"product_name"=>"".$arr[product][topic_en]."",
 ...
 <?  include ("mod/lab/popup_guest.php");?>
<?
$i++;	
}
$db->closedb ();
 ...
 */
 ...
 <? } ?>
 <div id="load_new<?=$page_start?>">load</div>