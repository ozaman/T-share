...
<br>
 ...
 <table width="100%" border="0" cellspacing="0" cellpadding="0"  >
                        <tr>
                          <td><a href="?name=content&file=shop">  สถานที่ช็อปปิ้ง</a> </td>
                        </tr>
                      </table>
...
$res[project] = $db->select_query("SELECT * FROM shopping_product  ORDER BY  id  ASC LIMIT $goto, $limit ");
...
?> 
 ...
 <td height="30" align="center">
        <a href="?name=content&file=shop&op=transferproduct_edit&id=<? echo $arr[project][id];?>" id="btn_sssmenu_edit_<? echo $arr[project][id];?>" >
    <i class="fa  fa-edit" style="width:40px; font-size:30px;"  ></i>
         ...
         <td align="center"> <a href="?name=content&file=shop&op=add_pic&id=<? echo $arr[project][id];?>" id="btn_sssmenu_edit_<? echo $arr[project][id];?>" >
    <i class="fa  fa-plus-circle" style="width:40px; font-size:30px;"  ></i>
  
        </a></td>
         <td align="center">
 
         <img src="images/shop_logo/<? echo $arr[project][logo_code];?>.png" alt="Edit" width="120" border="0" >  
...
"logo_code"=>"$_POST[logo_code]"
		...
		////$ProcessOutput .= "<A HREF=\"?name=content&file=shop\"><B>Back to Car Model </B></A>";
		$ProcessOutput .= "</CENTER>";
		$ProcessOutput .= "<BR><BR>";
//////echo "<meta http-equiv=refresh content=1;URL=?name=content&file=shop>";
//echo "<meta http-equiv=refresh content=0;URL=?name=content&file=shop&op=transferproduct_add>";
	}else{
		//óҹ
...
$arr[projecttype] = $db->fetch($res[projecttype]);
	...
	echo $_GET[type];
?>
  <FORM NAME="myform"   id="myform"  METHOD=POST ACTION="?name=content&file=shop&op=transferproduct_add&action=add" enctype="multipart/form-data">
      ...
      <tr>
        <td width="100%"><table width="100%" border="0" cellspacing="3" cellpadding="3">
		          ...
		          <tr>
		            ...
		            <td><input name="topic_en2" type="text" class="inputform" id="topic_en2" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][topic_en];?>" /></td>
		            </tr>
		          <tr>
		            <td><strong>TH :</strong></td>
		            <td><input name="topic_th2" type="text" class="inputform" id="topic_th2" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][topic_th];?>" /></td>
		            </tr>
		          <tr>
...
"logo_code"=>"$_POST[logo_code]"
	...
	///	$ProcessOutput .= "<A HREF=\"?name=admin/product&file=capacity\"><B>Back to Car Model</B></A>";
		$ProcessOutput .= "</CENTER>";
		$ProcessOutput .= "<BR><BR>";
	 echo "<meta http-equiv=refresh content=0;URL=?name=content&file=shop>";
	}else{
		//óҹ
		$ProcessOutput = $PermissionFalse ;
	}
...
<FORM NAME="myform" id="myform" METHOD=POST ACTION="?name=content&file=shop&op=transferproduct_edit&action=edit&id=<?=$_GET[id];?>" enctype="multipart/form-data">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
      ...
      <tr>
        <td width="100%"><table width="100%" border="0" cellspacing="3" cellpadding="3">
		          ...
		          <tr>
		            ...
		            <td><input name="logo_code3" type="text" class="inputform" id="logo_code3" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][logo_code];?>" /></td>
		            </tr>
	   ...
	   echo ">".$arr[category][topic_en]." </option>";
		          ...
		          <tr>
                    ...
                    <td><select name="message"   id="message"    style="font-size:14px; padding-bottom: 1px; padding: 5px;  background-color:#FFFDE9; height:30px; width:500px;" chosen width="'100%'">
                        <?  
                      $db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
                   $res[category] = $db->select_query("SELECT * FROM  shopping_product ORDER BY type,topic_en ");
while ($arr[category] = $db->fetch($res[category])){
...
$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
$res[projecttype] = $db->select_query("SELECT * FROM shopping_product where   id='".$_GET[id]."'  ");
$arr[projecttype] = $db->fetch($res[projecttype]);
...
?>
  <FORM NAME="myform"   id="myform"  METHOD=POST ACTION="?name=content&file=shop&op=add_pic_form&action=add&id=<?=$_GET[id]?>" enctype="multipart/form-data">
