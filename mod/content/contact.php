...
<br>
 ...
 <tr>
 <td><a href="?name=content&file=contact">  สถานที่ช็อปปิ้ง</a> &nbsp;&nbsp;<a href="?name=content&file=contact&op=transferproduct_add">  เพิ่ม</a></td>
                        </tr>
                      </table>
...
?>
   ...
   <table width="100%" cellspacing="2" cellpadding="1" >
    <tr bgcolor="#990000" height=25>
     <td width="30" align="center" bgcolor="#999999"><CENTER>
            <font color="#FFFFFF">Edit</font>     
     </CENTER></td>
     
   
     <td width="60" align="center" bgcolor="#999999"><font color="#FFFFFF">Pic</font></td>
     
   <td width="130" align="center" bgcolor="#999999"><font color="#FFFFFF">Logo</font></td>
     <td width="300" height="30" align="center" bgcolor="#999999"><font color="#FFFFFF">name</font></td>
     
   
     <td width="400" height="30" align="center" bgcolor="#999999"><font color="#FFFFFF">channel</font></td>
     <td width="400" height="30" align="center" bgcolor="#999999">&nbsp;</td>
...
$res[project] = $db->select_query("SELECT * FROM use_time  ORDER BY  id  ASC LIMIT $goto, $limit ");
...
?> 
 ...
 <td height="30" align="center">
        <a href="?name=content&file=contact&op=transferproduct_edit&id=<? echo $arr[project][id];?>" id="btn_sssmenu_edit_<? echo $arr[project][id];?>" >
    <i class="fa  fa-edit" style="width:40px; font-size:30px;"  ></i>
         ...
         <td align="center"> <a href="?name=content&file=contact&op=add_pic&id=<? echo $arr[project][id];?>" id="btn_sssmenu_edit_<? echo $arr[project][id];?>" >
    <i class="fa  fa-plus-circle" style="width:40px; font-size:30px;"  ></i>
  
        </a></td>
         <td align="center">&nbsp;</td>
         <td align="center">  
	     <a  onclick="responsiveVoice.speak(' <? echo $arr[project][topic_en];?>', 'UK English Female', {rate: 0.8});" > <? echo $arr[project][name];?></a></td>
         <td align="center">
	     
	 <a  onclick="responsiveVoice.speak(' <? echo $arr[project][topic_th];?>', 'Thai Female', {rate: 0.8});" > <? echo $arr[project][channel];?></a>  
	     
	   </td>
         <td align="center"><? echo $arr[type][phone];?></td>
...
////////////////
		...
		////$ProcessOutput .= "<A HREF=\"?name=content&file=contact\"><B>Back to Car Model </B></A>";
		$ProcessOutput .= "</CENTER>";
		$ProcessOutput .= "<BR><BR>";
 echo "<meta http-equiv=refresh content=1;URL=?name=content&file=contact>";
//echo "<meta http-equiv=refresh content=0;URL=?name=content&file=contact&op=transferproduct_add>";
	}else{
		//óҹ
...
$arr[projecttype] = $db->fetch($res[projecttype]);
	...
	echo $_GET[type];
?>
  <FORM NAME="myform"   id="myform"  METHOD=POST ACTION="?name=content&file=contact&op=transferproduct_add&action=add" enctype="multipart/form-data">
      ...
      <tr>
        <td width="100%"><table width="100%" border="0" cellspacing="3" cellpadding="3">
		          ...
		          <tr>
		            ...
		            <td><input name="product_id" type="text" class="inputform" id="product_id" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][topic_en];?>" /></td>
		            </tr>
		          <tr>
		            <td><strong>name :</strong></td>
		            <td><input name="name" type="text" class="inputform" id="name" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][topic_th];?>" /></td>
		            </tr>
		          <tr>
		            <td><strong>channel :</strong></td>
		            <td><input name="channel" type="text" class="inputform" id="channel" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][topic_cn];?>" /></td>
		            </tr>
		          <tr>
		            <td><strong>type :</strong></td>
		            <td><input name="type" type="text" class="inputform" id="type" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][logo_code];?>" /></td>
		            </tr>
		          <tr>
                    <td><strong>group :</strong></td>
		            <td><input name="group" type="text" class="inputform" id="group" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][topic_cn];?>" /> </td>
		          </tr>
		          <tr>
		            <td><strong>phone :</strong></td>
		            <td><input name="phone" type="text" class="inputform" id="phone" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][topic_cn];?>" /></td>
		            </tr>
          <tr>
            <td>&nbsp;</td>  
            <td>       
...
<?
	...
	///	$ProcessOutput .= "<A HREF=\"?name=admin/product&file=capacity\"><B>Back to Car Model</B></A>";
		$ProcessOutput .= "</CENTER>";
		$ProcessOutput .= "<BR><BR>";
	 echo "<meta http-equiv=refresh content=0;URL=?name=content&file=contact>";
	}else{
		//óҹ
		$ProcessOutput = $PermissionFalse ;
	}
	...
	if(1==1){
		//֧
		$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
		$res[project] = $db->select_query("SELECT * FROM use_time  WHERE id='".$_GET[id]."' ");
...
<FORM NAME="myform" id="myform" METHOD=POST ACTION="?name=content&file=contact&op=transferproduct_edit&action=edit&id=<?=$_GET[id];?>" enctype="multipart/form-data">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
      ...
      <tr>
        <td width="100%"><table width="100%" border="0" cellspacing="3" cellpadding="3">
		          ...
		          <tr>
                    <td><strong>channel :</strong></td>
                    <td><input name="channel" type="text" class="inputform" id="channel" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][channel];?>" /></td>
		            </tr>
		          <tr>
                    <td><strong>phone :</strong></td>
                    <td><input name="phone" type="text" class="inputform" id="phone" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][phone];?>" />
                    </td>
		            </tr>
		          <tr>
		            <td><strong>CODE:</strong></td>
		            <td><input name="logo_code" type="text" class="inputform" id="logo_code" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][logo_code];?>" /></td>
		            </tr>
	   ...
	   echo ">".$arr[category][topic_en]." </option>";
		          ...
		          <tr>
                    ...
                    <td><select name="message"   id="message"    style="font-size:14px; padding-bottom: 1px; padding: 5px;  background-color:#FFFDE9; height:30px; width:500px;" chosen width="'100%'">
                        <?  
                      $db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
                   $res[category] = $db->select_query("SELECT * FROM  use_time ORDER BY type,topic_en ");
while ($arr[category] = $db->fetch($res[category])){
...
$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
$res[projecttype] = $db->select_query("SELECT * FROM use_time where   id='".$_GET[id]."'  ");
$arr[projecttype] = $db->fetch($res[projecttype]);
...
?>
  <FORM NAME="myform"   id="myform"  METHOD=POST ACTION="?name=content&file=contact&op=add_pic_form&action=add&id=<?=$_GET[id]?>" enctype="multipart/form-data">
