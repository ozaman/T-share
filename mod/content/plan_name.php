...
<br>
 ...
 <tr>
 <td><a href="?name=content&file=plan_name">  สถานที่ช็อปปิ้ง</a> &nbsp;&nbsp;<a href="?name=content&file=plan_name&op=transferproduct_add">  เพิ่ม</a></td>
                        </tr>
                      </table>
...
// $res[com] = $db->select_query("SELECT * FROM plan_product_price_extra WHERE  from_number <= $number AND to_number >= $number and type='park' ");
/// $arr[type] = $db->fetch($res[com]);
	
	
	
	
	//echo  $arr[type][price_park];
...
$res[project] = $db->select_query("SELECT * FROM plan_product_price_name  ORDER BY  id  ASC LIMIT $goto, $limit ");
...
?> 
 ...
 <td height="30" align="center">
        <a href="?name=content&file=plan_name&op=transferproduct_edit&id=<? echo $arr[project][id];?>" id="btn_sssmenu_edit_<? echo $arr[project][id];?>" >
    <i class="fa  fa-edit" style="width:40px; font-size:30px;"  ></i>
         ...
         <td align="center"> <a href="?name=content&file=plan_name&op=add_pic&id=<? echo $arr[project][id];?>" id="btn_sssmenu_edit_<? echo $arr[project][id];?>" >
...
////////////////
		$db->add_db('plan_product_price_name',array(
		...
		////$ProcessOutput .= "<A HREF=\"?name=content&file=plan_name\"><B>Back to Car Model </B></A>";
		$ProcessOutput .= "</CENTER>";
		$ProcessOutput .= "<BR><BR>";
 echo "<meta http-equiv=refresh content=1;URL=?name=content&file=plan_name>";
//echo "<meta http-equiv=refresh content=0;URL=?name=content&file=plan_name&op=transferproduct_add>";
	}else{
		//óҹ
...
$arr[projecttype] = $db->fetch($res[projecttype]);
	...
	echo $_GET[type];
?>
  <FORM NAME="myform"   id="myform"  METHOD=POST ACTION="?name=content&file=plan_name&op=transferproduct_add&action=add" enctype="multipart/form-data">
  
  <table width="100%" border="0" cellspacing="0" cellpadding="3">
      <tr>
        <td width="100%"><table width="100%" border="0" cellspacing="3" cellpadding="3">
		          <tr>
		            <td width="150"><strong>EN : </strong></td>
		          ...
		          <tr>
		            ...
		            <td><input name="topic_cn2" type="text" class="inputform" id="topic_cn2" style="width:500px; background:#FFFFFF" value="<? echo $arr[project][topic_cn];?>" /></td>
		            </tr>
          <tr>
            <td>&nbsp;</td>  
            <td>       
...
<?
	...
	if(1==1){
		...
		$db->update_db('plan_product_price_name',array(
	...
	///	$ProcessOutput .= "<A HREF=\"?name=admin/product&file=capacity\"><B>Back to Car Model</B></A>";
		$ProcessOutput .= "</CENTER>";
		$ProcessOutput .= "<BR><BR>";
	 echo "<meta http-equiv=refresh content=0;URL=?name=content&file=plan_name>";
	}else{
		//óҹ
		$ProcessOutput = $PermissionFalse ;
	}
	...
	if(1==1){
		//֧
		$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
		$res[project] = $db->select_query("SELECT * FROM plan_product_price_name  WHERE id='".$_GET[id]."' ");
...
<FORM NAME="myform" id="myform" METHOD=POST ACTION="?name=content&file=plan_name&op=transferproduct_edit&action=edit&id=<?=$_GET[id];?>" enctype="multipart/form-data">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
...
////echo "<meta http-equiv=refresh content=0;URL=?name=content&file=plan_name>";
	...
	if(1==1){
		$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
		$db->del('plan_product_price_name'," id='".$_GET[id]."' "); 
	   ...
	   echo ">".$arr[category][topic_en]." </option>";
		          ...
		          <tr>
                    ...
                    <td><select name="message"   id="message"    style="font-size:14px; padding-bottom: 1px; padding: 5px;  background-color:#FFFDE9; height:30px; width:500px;" chosen width="'100%'">
                        <?  
                      $db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
                   $res[category] = $db->select_query("SELECT * FROM  plan_product_price_name ORDER BY type,topic_en ");
while ($arr[category] = $db->fetch($res[category])){
...
$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
$res[projecttype] = $db->select_query("SELECT * FROM plan_product_price_name where   id='".$_GET[id]."'  ");
$arr[projecttype] = $db->fetch($res[projecttype]);
...
?>
  <FORM NAME="myform"   id="myform"  METHOD=POST ACTION="?name=content&file=plan_name&op=add_pic_form&action=add&id=<?=$_GET[id]?>" enctype="multipart/form-data">
