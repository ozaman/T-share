...
<br>
 ...
 <tr>
 <td><a href="?name=content&file=plan_list">  สถานที่ช็อปปิ้ง</a> &nbsp;&nbsp;<a href="?name=content&file=plan_list&op=transferproduct_add">  เพิ่ม</a></td>
                        </tr>
                      </table>
...
$res[project] = $db->select_query("SELECT * FROM plan_product_price_list  ORDER BY  id  ASC LIMIT $goto, $limit ");
...
?> 
 ...
 <td height="30" align="center">
        <a href="?name=content&file=plan_list&op=transferproduct_edit&id=<? echo $arr[project][id];?>" id="btn_sssmenu_edit_<? echo $arr[project][id];?>" >
    <i class="fa  fa-edit" style="width:40px; font-size:30px;"  ></i>
         ...
         <td align="center"> <a href="?name=content&file=plan_list&op=add_pic&id=<? echo $arr[project][id];?>" id="btn_sssmenu_edit_<? echo $arr[project][id];?>" >
...
////////////////
		$db->add_db('plan_product_price_list',array(
		...
		////$ProcessOutput .= "<A HREF=\"?name=content&file=plan_list\"><B>Back to Car Model </B></A>";
		$ProcessOutput .= "</CENTER>";
		$ProcessOutput .= "<BR><BR>";
 echo "<meta http-equiv=refresh content=1;URL=?name=content&file=plan_list>";
//echo "<meta http-equiv=refresh content=0;URL=?name=content&file=plan_list&op=transferproduct_add>";
	}else{
		//óҹ
...
$arr[projecttype] = $db->fetch($res[projecttype]);
	...
	echo $_GET[type];
?>
  <FORM NAME="myform"   id="myform"  METHOD=POST ACTION="?name=content&file=plan_list&op=transferproduct_add&action=add" enctype="multipart/form-data">
...
<?
	...
	if(1==1){
		...
		$db->update_db('plan_product_price_list',array(
	...
	///	$ProcessOutput .= "<A HREF=\"?name=admin/product&file=capacity\"><B>Back to Car Model</B></A>";
		$ProcessOutput .= "</CENTER>";
		$ProcessOutput .= "<BR><BR>";
	 echo "<meta http-equiv=refresh content=0;URL=?name=content&file=plan_list>";
	}else{
		//óҹ
		$ProcessOutput = $PermissionFalse ;
	}
	...
	if(1==1){
		//֧
		$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
		$res[project] = $db->select_query("SELECT * FROM plan_product_price_list  WHERE id='".$_GET[id]."' ");
...
<FORM NAME="myform" id="myform" METHOD=POST ACTION="?name=content&file=plan_list&op=transferproduct_edit&action=edit&id=<?=$_GET[id];?>" enctype="multipart/form-data">
    <table width="100%" border="0" cellspacing="0" cellpadding="0">
      <tr>
...
////echo "<meta http-equiv=refresh content=0;URL=?name=content&file=plan_list>";
	...
	if(1==1){
		$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
		$db->del('plan_product_price_list'," id='".$_GET[id]."' "); 
	   ...
	   echo ">".$arr[category][topic_en]." </option>";
		          ...
		          <tr>
                    ...
                    <td><select name="message"   id="message"    style="font-size:14px; padding-bottom: 1px; padding: 5px;  background-color:#FFFDE9; height:30px; width:500px;" chosen width="'100%'">
                        <?  
                      $db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
                   $res[category] = $db->select_query("SELECT * FROM  plan_product_price_list ORDER BY type,topic_en ");
while ($arr[category] = $db->fetch($res[category])){
...
$db->connectdb(DB_NAME_APP,DB_USERNAME,DB_PASSWORD);
$res[projecttype] = $db->select_query("SELECT * FROM plan_product_price_list where   id='".$_GET[id]."'  ");
$arr[projecttype] = $db->fetch($res[projecttype]);
...
?>
  <FORM NAME="myform"   id="myform"  METHOD=POST ACTION="?name=content&file=plan_list&op=add_pic_form&action=add&id=<?=$_GET[id]?>" enctype="multipart/form-data">
