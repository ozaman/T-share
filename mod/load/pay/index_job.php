...
//  $res[bookcheck] = $db->select_query("SELECT  * FROM  ap_order WHERE driver = '".$_GET[driver]."' ");
...
$db = New DB();
	$db->connectdb('admin_booking','admin_MANbooking','252631MANbooking');
$query = "SELECT SUM(total_price)  FROM  ap_order $data_sale "; 
$result = mysql_query($query) or die(mysql_error());
// Print out result
while($row = mysql_fetch_array($result)){
$pay_advance= $row['SUM(total_price)'];
}

 
 $db = New DB();
	$db->connectdb('admin_booking','admin_MANbooking','252631MANbooking');
$query = "SELECT  total_price  FROM ap_order $data_sale_today "; 
$result = mysql_query($query) or die(mysql_error());
// Print out result
$pay_total = 0;
while($row = mysql_fetch_array($result)){
