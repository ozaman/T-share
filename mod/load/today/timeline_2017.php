...
///   $( ".bottom_popup" ).slideToggle();
  $( "#load_data_manage_work_show" ).toggle();
 	var daytype = '<?=$_GET[daytype]?>';
 	var id = '<?=$arr[project][id];?>';
 	var day = '<?=$transfer_date_show;?>';
 	var carno = '<?=$arr[project][carno];?>';
 	var cartype = '<?= $arr[project][cartype];?>';
 	var car_list = '<?=$i;?>';
 	var sv = '<?=$arr[project][server];?>';
 	var admin_company = '<?=$admin_company;?>';
 	var outdate_bkk = '<?=$outdate_bkk;?>';
 	
 	
 	var url_chat = "load_page_show.php?name=view&file=show&daytype="+daytype+"&id="+id+"&day="+day+"&carno="+carno+"&cartype="+cartype+"&car_list="+car_list+"&sv="+sv+"&admin_company="+admin_company+"&outdate_bkk="+outdate_bkk;
/// $( "#load_data_manage_work" ).toggle();
//    $('#load_data_manage_work_show').load('load/loading/page_main.php'); 
		console.log(url_chat);
//		$('#load_data_manage_work_show').load(url_chat); 
		$.post( url_chat, function( data ) {
		  $('#load_data_manage_work_show').html( data );
		});
