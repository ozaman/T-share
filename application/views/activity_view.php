...
<?php 
	...
	$query = $this->db->query("SELECT t1.*,t2.s_topic as ac_topic, t2.s_icons, t2.s_material_icons, t2.s_color FROM activity_event as t1 left join activity_menu as t2 on t1.i_type = t2.id where t1.i_driver = ".$_COOKIE[detect_user]." and t1.i_status = 1 order by t1.s_post_date desc ");
	$check_before = '';
	$check_now = '';
...
//		echo date('Y/m/d H:i:s',$row->s_post_date);
	?>
	    <div class="card-activity" id="card-ac_<?=$row->id;?>" <?=$bg;?> >
	    	<table width="100%">
	    		<tr>
	    			<td onclick="openActivity('<?=$row->id;?>', '<?=$row->i_type;?>', '<?=$row->ac_topic;?>', '<?=$row->i_event;?>');">
	    			...
	    			<td width="50" align="center" valign="middle" onclick="app.showFromTemplate();$('#id_activity_select').val(<?=$row->id;?>);">
