...
//            	 $query_em = $this->db->get('web_emergency_person');
        ...
        <ons-list-item class="input-items list-item p-l-0">
            ...
            <label class="center list-item__center">
                <ons-input id="email-input" float="" placeholder="<?=$email;?>" name="email" style="width:100%;" value="<?=$val->email;?>">
        ...
        <div align="center">
	      ...
	      <div class="box-preview-img" id="box_img_profile"  style="width: 170px;height: 170px;">
	      	<img src="../data/pic/driver/small/<?=$driver->username;?>.jpg" style="max-width: 100%; height: 170px;" id="pv_profile" onclick="performClick('img_profile');"  /><br/>
...
/*    bottom: 278px;*/
    ...
    border-top-left-radius: 5px; pointer-events: none;" ><i class="fa fa-camera" aria-hidden="true"></i>&nbsp; อัพโหลดรูปถ่าย</span>
      ...
      <div align="center" style="margin: 10px;">
	      ...
	      <div class="box-preview-img" id="box_img_id_drving" >
	      	<img src="assets/images/ex_card/id_driving.jpg" class="img-preview-show" id="pv_id_drving" onclick="performClick('img_id_drving');" />
	      </div> 
	      <span style="background-color: #f4f4f4;
    padding: 0px 10px;
    position: absolute;
    ...
    <ons-button modifier="outline" class="button-margin button button--outline button--large" onclick="createDialogPf();" style="background-color: #fff;" >ยืนยันข้อมูล</ons-button>
    </div>
...
<script>
	 ...
	 $.ajax({
			url: '../data/pic/driver/id_card/<?=$driver->id;?>_idcard.jpg',
			type:'HEAD',
			error: function()
			{
...
//				 $('#idcard_img').show();
				 ...
				 $('#pv_id_card').attr('src','../data/pic/driver/id_card/<?=$driver->id;?>_idcard.jpg?v=<?=time();?>');
			}
		});
		
	  $.ajax({
			url: '../data/pic/driver/id_driving/<?=$driver->id;?>_iddriving.jpg',
			type:'HEAD',
			error: function()
			{
			   console.log('Error file');
...
//				$('#iddriving_img').show();
				...
				$('#pv_id_drving').attr('src','../data/pic/driver/id_driving/<?=$driver->id;?>_iddriving.jpg?v=<?=time();?>');
			}
		});
	var num = 0;
